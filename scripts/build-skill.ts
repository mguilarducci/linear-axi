// Generates skills/linear-axi/SKILL.md from the shared CLI guidance so the
// installable skill never drifts from what `linear-axi` prints.
//
...
if (check) {
  ...
  if (actual !== expected) {
    console.error(
      "skills/linear-axi/SKILL.md is out of date. Run `pnpm run build:skill` and commit the result.",
    );
    process.exit(1);
  }
